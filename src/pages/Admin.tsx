...
export function Admin() {
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'plants' | 'packages' | 'orders' | 'returns' | 'rentals'
  >('dashboard');
  const { updateRentalStatus, assignDeliveryPerson } = useRental();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${
                adminStats.ordersChange > 0 ? 'text-green-600' : 'text-red-600'
              }`}
            >
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${
                adminStats.revenueChange > 0 ? 'text-green-600' : 'text-red-600'
              }`}
            >
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-sm text-gray-500 mb-1">Plant Packages</p>
          <p className="font-display text-3xl font-bold text-gray-900">
            {plantPackages.length}
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-sm text-gray-500 mb-1">Rental Requests</p>
          <p className="font-display text-3xl font-bold text-gray-900">
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 text-sm text-gray-500">
            <tr>
              ...
              <th className="p-4 font-medium">Sale Stock</th>
              <th className="p-4 font-medium">Rental Stock</th>
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="flex gap-2">
                <button className="flex-1 px-3 py-2 text-sm font-medium text-green-600 hover:bg-green-50 rounded-lg transition-colors flex items-center justify-center gap-1">
                  <EditIcon size={14} /> Edit
                </button>
                <button className="flex-1 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors flex items-center justify-center gap-1">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {request.deliveryAddress.city}, {request.deliveryAddress.district}
              </p>
            </div>
          </div>

          <div className="mb-4 pb-4 border-b border-gray-200">
            <p className="text-sm font-semibold text-gray-700 mb-2">Plants</p>
            <div className="space-y-1">
              {request.plants.map((plant) => (
                <div key={plant.plantId} className="text-sm text-gray-600 flex justify-between">
                  <span>Plant ID {plant.plantId} x {plant.quantity}</span>
          ...
          {request.status === 'pending' && (
            <div className="flex gap-2 mb-4">
              <button
                onClick={() => updateRentalStatus(request.id, 'approved')}
                className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg flex items-center justify-center gap-2 transition-colors"
              >
                <CheckCircleIcon size={18} /> Approve
              </button>
              <button
                onClick={() => updateRentalStatus(request.id, 'rejected')}
                className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg flex items-center justify-center gap-2 transition-colors"
              >
          ...
          {request.status === 'approved' && !request.assignedDeliveryPerson && (
            <div className="flex gap-2">
              <input
                type="text"
              ...
              <button
                onClick={() => {
                  const input = (
                    document.getElementById(`delivery-${request.id}`) as HTMLInputElement
                  )?.value;
                  if (input) assignDeliveryPerson(request.id, input);
                }}
      ...
      <div className="w-full md:w-64 bg-white border-r border-gray-200 shrink-0 md:min-h-screen">
        ...
        <nav className="px-4 space-y-1">
          {[
            ...
            { id: 'plants', label: 'Manage Plants', icon: PackageIcon },
            { id: 'packages', label: 'Plant Packages', icon: PackageIcon },
            { id: 'orders', label: 'Orders', icon: ShoppingBagIcon },
            { id: 'returns', label: 'Returns', icon: RefreshCcwIcon },
            { id: 'rentals', label: 'Rentals', icon: TruckIcon }
          ].map((item) => (
            <button
              key={item.id}
